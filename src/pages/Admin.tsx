...
import { Lock, LogOut, Eye, CheckCircle } from 'lucide-react';
...
function AdminProducts() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const otherImgs = form.otherImages.split(',').map(s=>s.trim()).filter(Boolean);
    const data = {
      name: form.name,
      ...
      images: [form.imageUrl, ...otherImgs],
  ...
  const openModal = (p?: Product) => {
    if (p) {
      ...
      setForm({
        ...
        category: p.category,
        imageUrl: p.imageUrl,
        otherImages: p.images ? p.images.slice(1).join(', ') : '',
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-3xl rounded-[3rem] p-8 md:p-12 max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                   <label className="block text-sm font-bold mb-2">الصورة الرئيسية (URL إلزامي)</label>
                   <input required value={form.imageUrl} onChange={e => setForm({...form, imageUrl: e.target.value})} className="w-full bg-zinc-50 border border-zinc-200 focus:border-black rounded-2xl px-6 h-14 outline-none font-bold text-left" dir="ltr" />
                </div>
                <div className="md:col-span-2">
                   <label className="block text-sm font-bold mb-2">صور إضافية (اختياري - روابط مفصولة بفاصلة)</label>
                   <textarea value={form.otherImages} onChange={e => setForm({...form, otherImages: e.target.value})} className="w-full bg-zinc-50 border border-zinc-200 focus:border-black rounded-2xl px-6 py-4 outline-none font-bold text-left" dir="ltr" rows={2} />
                </div>
                <div className="md:col-span-2">
